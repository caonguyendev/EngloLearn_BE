...
const sendEmail = async ({ to, subject, text, html, ...rest }) => {
  try {
    ...
    if (res) {
      ...
      const mail = {
        //sender access
        from: '"EngloLearn" <no-reply@accounts.dynonary.com>',
        //receiver access
        to,
        //subject
        subject,
...
const htmlSignupAccount = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Mã xác nhận đăng ký tài khoản cho website EngloLearn của bạn.<br />
...
const htmlResetPassword = (token) => {
  return `<div>
    ...
    <h1 style="background: #eee; corlor: #018c4c; padding: 10px;">
      <i><b>${token}</b></i>
    </h1>
    ${footerHtmlVerifyMail}
  </div>`;
